...
try:
    ...
    # Intentar importar FontManager si está disponible
    try:
        from matplotlib.font_manager import FontManager
    except ImportError:
        # Si no está disponible, no es crítico para guardar la figura
        pass

    # Guardar la figura actual si existe
    if plt.get_fignums():
        plt.savefig('last_plot.png', bbox_inches='tight', dpi=150)
        print("Figura guardada como 'last_plot.png'")
    else:
        print("No se encontraron figuras activas")

except Exception as e:
    print(f"Error al guardar la figura: {str(e)}")
